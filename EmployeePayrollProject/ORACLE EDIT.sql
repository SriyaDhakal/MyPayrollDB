-- calculating the percentage of emplpoyees in each department raltive to the total number of all employees
-- Inline views
...
SELECT
    D.Department_Name,
    COUNT(E.Employee_Id) AS EmployeesInDepartment,
    (COUNT(E.Employee_Id) / TotalEmployees.TotalCount * 100) AS PercentageOfTotalEmployees
FROM
    Department AS D
JOIN
    Employee AS E ON D.Department_Id = E.Department_Id
CROSS JOIN -- Use CROSS JOIN to bring in the total count for every row
    (SELECT COUNT(*) AS TotalCount FROM Employee WHERE Department_Id IS NOT NULL) AS TotalEmployees
GROUP BY
    D.Department_Name, TotalEmployees.TotalCount -- Group by department name and the constant total count
ORDER BY
    PercentageOfTotalEmployees DESC;
    
    
    -- number of employee with different degrees
    
    
    SELECT Degree, 
    count(Degree) as Employeeswithdegree
    from EDUCATION 
    group by Degree;
    Inline Views
-----------------------------------
...
pragmam exception_init(null_violated_exception , -1400);
